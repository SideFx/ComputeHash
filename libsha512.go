...
//export GetSHAOutputSize
func GetSHAOutputSize() int { return sha512size }

//export Compute
func Compute(p []byte, result []byte) {
	...
	var d0 [shaRounds]uint64
	var v0 [8]uint64
	var size = uint64(len(p))
	var sd = size/shaBlockSize + 1
	if (size % shaBlockSize) >= (shaBlockSize - shaReservedBytes) {
		sd++
	}
	// get number of bits
	sl, sh = size<<3, size>>61
	mc := make([]byte, size)
	copy(mc, p)
	mc = append(mc, 0x80)
	for i = size + 1; i < sd*shaBlockSize-shaReservedBytes; i++ {
		mc = append(mc, 0x00)
	}
	...
	for n = 0; n < sd; n++ {
		...
		for i = 0; i < 16; i++ {
			...
			d0[i] = (v0[0] << 56) | (v0[1] << 48) | (v0[2] << 40) | (v0[3] << 32) |
				(v0[4] << 24) | (v0[5] << 16) | (v0[6] << 8) | v0[7]
			z += 8
		}
		for i = 16; i < shaRounds; i++ {
			sl = ror64(d0[i-15], 1) ^ ror64(d0[i-15], 8) ^ (d0[i-15] >> 7)
			sh = ror64(d0[i-2], 19) ^ ror64(d0[i-2], 61) ^ (d0[i-2] >> 6)
			d0[i] = d0[i-16] + sl + d0[i-7] + sh
		}
		for i = 0; i < shaRounds; i++ {
			sh = ror64(he, 14) ^ ror64(he, 18) ^ ror64(he, 41)
			sc = (he & hf) ^ ((^he) & hg)
			t0 = hh + sh + sc + k0[i] + d0[i]
	...
	i = 0
	for j = 0; j < 8; j++ {
		result[i] = byte(h0[j] >> 56)
		result[i+1] = byte(h0[j] >> 48)
		result[i+2] = byte(h0[j] >> 40)
		result[i+3] = byte(h0[j] >> 32)
		result[i+4] = byte(h0[j] >> 24)
		result[i+5] = byte(h0[j] >> 16)
		result[i+6] = byte(h0[j] >> 8)
		result[i+7] = byte(h0[j])
		i += 8
	}
}
